...
def linear_design_matrix(ind_vars):
    ...
    """
    # TODO: Fill in this function and change the return value
    # col = ind_vars.shape[1]
    # B_i = np.ones(ind_vars.shape[0]); # Beta_0
    # for i in range(1, col): # B_i 
    #     B_i = np.column_stack((B_i, B_i[:,i] * ind_vars[:,i]))
...
def quadratic_design_matrix(ind_vars):
    ...
    """
    # TODO: Fill in this function and change the return value
    quad = linear_design_matrix(ind_vars)
    
    col = ind_vars.shape[1]
    for i in range(0, col): # B_i
        for j in range(i, col) :
            quad = np.column_stack((quad, ind_vars[:, i] * ind_vars[:, j]))
...
def cubic_design_matrix(ind_vars):
    ...
    cubic = quadratic_design_matrix(ind_vars);
    
    for i in range(0, col):
        for j in range(i, col) :
            for k in range(j, col) :
                cubic = np.column_stack((cubic, ind_vars[:, i] * ind_vars[:, j] * ind_vars[:, k]))
